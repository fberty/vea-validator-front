...
#[tokio::test]
#[serial]
async fn test_challenge_when_claim_has_incorrect_root_arb_to_gnosis() {
    println!("\n==========================================================");
    println!("CHALLENGE TEST (ARB->GNOSIS): Validator Challenges Bad Claim");
    println!("==========================================================\n");
    ...
    let weth_addr = route.weth_address.expect("Gnosis route should have WETH");
    ...
    if logs.is_empty() {
        panic!("VALIDATOR FAILED: Did not challenge the bad claim on ARB->GNOSIS route");
    }
    ...
    println!("\nCHALLENGE TEST (ARB->GNOSIS) PASSED!");
    println!("Validator correctly detected invalid claim and challenged it");
}
